...
function LeftPanel() {
  ...
  function leftpanelview() {
    if (selectedHeader == "question")
      return <List>{listQuestionLeftPanel}</List>;
    else
      return (
        <div>
          <InputBase
            className='leftpanel_title_changer'
  ...
  Object.keys(Mquestion).forEach(function (key) {
    listQuestionLeftPanel[
      listQuestionLeftPanel.length
    ] = QuestionListGeneratingFunc(Mquestion[key]);
  });
